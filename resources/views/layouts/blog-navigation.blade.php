<header class="bg-white px-6 shadow dark:bg-slate-900">
    <div class="mx-auto flex h-16 max-w-6xl items-center justify-between">
        ...
        <a class="hidden text-sky-500 duration-200 hover:rotate-6 md:block" href="/">
        ...
        <div class="-mr-4 flex items-center">
            <a class="text-sky-500 duration-200 hover:rotate-6 md:hidden" href="/">
                <svg class="h-8 w-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                    xmlns="http://www.w3.org/2000/svg">
                    <path d="M12 14l9-5-9-5-9 5 9 5z"></path>
                    <path
            ...
            <div class="ml-8 hidden space-x-8 md:flex">
                <a class="text-sm font-medium {{ request()->routeIs('home') ? 'text-sky-500' : 'text-slate-600 hover:text-sky-500 dark:text-slate-400 dark:hover:text-sky-500' }}"
                    href="{{ route('home') }}">Home</a>
                <a class="text-sm font-medium {{ request()->routeIs('about') ? 'text-sky-500' : 'text-slate-600 hover:text-sky-500 dark:text-slate-400 dark:hover:text-sky-500' }}"
                    href="{{ route('about') }}">About</a>
                <a class="text-sm font-medium {{ request()->routeIs('post.index') ? 'text-sky-500' : 'text-slate-600 hover:text-sky-500 dark:text-slate-400 dark:hover:text-sky-500' }}"
                    href="{{ route('post.index') }}">Blog</a>
                <a class="text-sm font-medium {{ request()->routeIs('contact') ? 'text-sky-500' : 'text-slate-600 hover:text-sky-500 dark:text-slate-400 dark:hover:text-sky-500' }}"
        ...
        <div class="flex">
            ...
            <button
                ...
                <img class="h-6 w-6 rounded-full" src="https://ui-avatars.com/api?name=Jorge+Garcia"
                    alt="Jorge García" />
            </button>
        </div>
    </div>
   <div id="mobile-menu" class="hidden space-y-1 border-t pb-3 pt-2 dark:border-slate-500">
        <a class="block px-3 py-2 {{ request()->routeIs('home') ? 'text-sky-500' : 'text-slate-600 hover:text-sky-500 dark:text-slate-400 dark:hover:text-sky-500' }}"
            href="{{ route('about') }}">Home</a>
        <a class="block px-3 py-2 {{ request()->routeIs('about') ? 'text-sky-500' : 'text-slate-600 hover:text-sky-500 dark:text-slate-400 dark:hover:text-sky-500' }}"
            href="{{ route('about') }}">Abouts</a>
        <a class="block px-3 py-2 {{ request()->routeIs('about') ? 'text-sky-500' : 'text-slate-600 hover:text-sky-500 dark:text-slate-400 dark:hover:text-sky-500' }}"
            href="{{ route('post.index') }}">Blog</a>
        <a class="block px-3 py-2 {{ request()->routeIs('about') ? 'text-sky-500' : 'text-slate-600 hover:text-sky-500 dark:text-slate-400 dark:hover:text-sky-500' }}"
            href="{{ route('contact') }}">Contact</a>
